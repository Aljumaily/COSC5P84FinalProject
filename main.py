...
def main():
    ...
    parser.add_argument("--lm_epochs", type=int, default=0)
    parser.add_argument("--lm_warmup_steps", type=int, default=500)
    parser.add_argument("--epochs", type=int, default=5)
    parser.add_argument("--lm_batch_size", type=int, default=16)
    ...
    parser.add_argument("--prefix_hidden_size", type=int, default=800)

    # loss args
    parser.add_argument("--alpha1", type=int, default=1)
    parser.add_argument("--alpha2", type=int, default=50)
    parser.add_argument("--alpha3", type=int, default=200)
    parser.add_argument("--alpha4", type=int, default=250)

    # alpha5 is discarded
    parser.add_argument("--alpha5", type=int, default=0)
    parser.add_argument("--beta", type=int, default=10)
    parser.add_argument("--random_seed", type=int, default=42)
    parser.add_argument("--use_full_words", default=True)
    ...
    # store the langauge key separately and remove it from the dictionary (as this is a cutom key and will generate an error if it is still included).
    parameters_without_language: dict = vars(args)
    language: str = vars(args)['language']
    parameters_without_language.pop("language")
    config.update(parameters_without_language)
    print(sys.argv, len(sys.argv))
    # print(args.en, args.fr)
    ...
    # Check if en, then load in the original data: en-en-data.txt and en-en-gender_swapped_data.txt
    # Check if fr, then load in the original data: fr-data.txt and fr-gender_swapped_data.txt, the french version
    # Check if fr-en, then load in the fr-en-data.txt and fr-en-gender_swapped_data.txt, the translated english version of the french.
    # Check if es, then load in the original data: es-data.txt and es-gender_swapped_data.txt, the spanish version
    # Check if es-en, then load in the es-en-data.txt and es-en-gender_swapped_data.txt, the translated english version of the spanish.
    # Store the paths of each male.txt, female.txt, data.txt, etc. into variables and then below, instead of hardcoding the paths, use:
    # male_words = load_file_to_list(male_data_file), where male_data_file contains the path of the appropirate langauge. 
    #
    #
    
    data_path: str = ""
    gender_swapped_data_path: str = ""
    male_words_path: str = ""
    female_words_path: str = ""
    neutral_words_path: str = ""
    male_words_path: str = ""
    male_words_path: str = ""
    male_words_path: str = ""
    if language == "en":
        ...
        print("fr is selected")
    elif language == "fr-en":
        print("fr-en is selected")
    elif language == "es":
        print("es is selected")
    elif language == "es-en":
        print("es-en is selected")
    else:
        language = "en"
        print("Invalid language selected, 'en' is selected instead.")
    
    data_path = os.path.join(config.data_dir, f'{language}/data.txt')
    gender_swapped_data_path = os.path.join(config.data_dir, f'{language}/gender_swapped_data.txt')
    ...
    # load data
    male_words = load_file_to_list(f'data/{language}/male.txt')
    female_words = load_file_to_list(f'data/{language}/female.txt')
    neutral_words = list(set(load_file_to_list(f'data/{language}/neutral.txt')))
    ...
    with open(log_file, 'w', encoding='utf-8') as f:
        ...
        for epoch in range(lm_epochs):
            ...
            print(f"\ntrain total loss: {sum(train_total_losses)/len(train_total_losses)}", file=f)
            print(f"train lm loss: {sum(train_lm_losses)/len(train_lm_losses)}", file=f)
            
            print(f"\ntrain total loss: {sum(train_total_losses)/len(train_total_losses)}")
...
if __name__ == '__main__' :
    main()
    # parser = argparse.ArgumentParser()
    # # parser.add_argument('-en', '--english', action=argparse.BooleanOptionalAction, default=True, help='Debiasing the English language.')
    # # parser.add_argument('-fr', '--french', action=argparse.BooleanOptionalAction, default=False, help='Debiasing the French language.')
    # parser.add_argument('-lang', '--langauge', default='en', type=str, help='Language name: en or fr.')
    # args = parser.parse_args()
    # if args.langauge not in ['en', 'fr']:
    #     print("please use the argument -lang=en or -lang=fr.")
    # else:
    #     pass
